...
def _validate_bounds(bounds: Bounds) -> list[tuple[float, float]]:
    """Check that the search space is valid for the two input variables."""
    validated_bounds = list(bounds)
    if len(validated_bounds) != 2:
        raise ValueError("bounds must define exactly two feature ranges.")
...
def get_next_experiment(
    ...
    X_grid = _build_grid(bounds, resolution)
    ei = calculate_ei(gp_model, X_grid, y_best=float(np.max(y_array)))
